...
impl Method for Start {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Start> {
        if method_frame.class_id != 10 || method_frame.method_id != 10 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let version_major = reader.read_byte().unwrap();
        let version_minor = reader.read_byte().unwrap();
        let server_properties = decode_table(&mut reader).unwrap();
        let size = reader.read_be_u32().unwrap() as uint;
        let mechanisms = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_be_u32().unwrap() as uint;
        let locales = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(Start { version_major: version_major, version_minor: version_minor, server_properties: server_properties, mechanisms: mechanisms, locales: locales })
    }
    fn encode(&self) -> Vec<u8> {
...
impl Method for StartOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<StartOk> {
        if method_frame.class_id != 10 || method_frame.method_id != 11 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let client_properties = decode_table(&mut reader).unwrap();
        let size = reader.read_byte().unwrap() as uint;
        let mechanism = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_be_u32().unwrap() as uint;
        let response = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_byte().unwrap() as uint;
        let locale = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(StartOk { client_properties: client_properties, mechanism: mechanism, response: response, locale: locale })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for Secure {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Secure> {
        if method_frame.class_id != 10 || method_frame.method_id != 20 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_be_u32().unwrap() as uint;
        let challenge = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(Secure { challenge: challenge })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for SecureOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<SecureOk> {
        ...
        let size = reader.read_be_u32().unwrap() as uint;
        let response = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(SecureOk { response: response })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u32(self.response.len() as u32).unwrap();
...
impl Method for Tune {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Tune> {
        if method_frame.class_id != 10 || method_frame.method_id != 30 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let channel_max = reader.read_be_u16().unwrap();
        let frame_max = reader.read_be_u32().unwrap();
        let heartbeat = reader.read_be_u16().unwrap();
        Some(Tune { channel_max: channel_max, frame_max: frame_max, heartbeat: heartbeat })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for TuneOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<TuneOk> {
        ...
        let frame_max = reader.read_be_u32().unwrap();
        let heartbeat = reader.read_be_u16().unwrap();
        Some(TuneOk { channel_max: channel_max, frame_max: frame_max, heartbeat: heartbeat })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.channel_max).unwrap();
...
impl Method for Open {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Open> {
        if method_frame.class_id != 10 || method_frame.method_id != 40 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_byte().unwrap() as uint;
        let virtual_host = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_byte().unwrap() as uint;
        let capabilities = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        let insist = bits.get(0);
        Some(Open { virtual_host: virtual_host, capabilities: capabilities, insist: insist })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for OpenOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<OpenOk> {
        if method_frame.class_id != 10 || method_frame.method_id != 41 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_byte().unwrap() as uint;
        let known_hosts = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(OpenOk { known_hosts: known_hosts })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for Close {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Close> {
        if method_frame.class_id != 10 || method_frame.method_id != 50 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let reply_code = reader.read_be_u16().unwrap();
        let size = reader.read_byte().unwrap() as uint;
        let reply_text = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let class_id = reader.read_be_u16().unwrap();
        let method_id = reader.read_be_u16().unwrap();
        Some(Close { reply_code: reply_code, reply_text: reply_text, class_id: class_id, method_id: method_id })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for CloseOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<CloseOk> {
...
impl Method for Blocked {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Blocked> {
        if method_frame.class_id != 10 || method_frame.method_id != 60 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_byte().unwrap() as uint;
        let reason = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
...
impl Method for Unblocked {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Unblocked> {
...
impl Method for Open {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Open> {
        ...
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_byte().unwrap() as uint;
        let out_of_band = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(Open { out_of_band: out_of_band })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for OpenOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<OpenOk> {
        ...
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_be_u32().unwrap() as uint;
        let channel_id = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(OpenOk { channel_id: channel_id })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for Flow {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Flow> {
...
impl Method for FlowOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<FlowOk> {
...
impl Method for Request {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Request> {
        if method_frame.class_id != 30 || method_frame.method_id != 10 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_byte().unwrap() as uint;
        let realm = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        ...
        let read = bits.get(4);
        Some(Request { realm: realm, exclusive: exclusive, passive: passive, active: active, write: write, read: read })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for RequestOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<RequestOk> {
        if method_frame.class_id != 30 || method_frame.method_id != 11 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let ticket = reader.read_be_u16().unwrap();
        Some(RequestOk { ticket: ticket })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for Declare {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Declare> {
        ...
        let ticket = reader.read_be_u16().unwrap();
        let size = reader.read_byte().unwrap() as uint;
        let exchange = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_byte().unwrap() as uint;
        let _type = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        ...
        let arguments = decode_table(&mut reader).unwrap();
        Some(Declare { ticket: ticket, exchange: exchange, _type: _type, passive: passive, durable: durable, auto_delete: auto_delete, internal: internal, nowait: nowait, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for DeclareOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<DeclareOk> {
...
impl Method for Delete {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Delete> {
        ...
        Some(Delete { ticket: ticket, exchange: exchange, if_unused: if_unused, nowait: nowait })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for DeleteOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<DeleteOk> {
...
impl Method for Bind {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Bind> {
        ...
        let ticket = reader.read_be_u16().unwrap();
        let size = reader.read_byte().unwrap() as uint;
        let destination = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_byte().unwrap() as uint;
        let source = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_byte().unwrap() as uint;
        let routing_key = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        let nowait = bits.get(0);
        let arguments = decode_table(&mut reader).unwrap();
        Some(Bind { ticket: ticket, destination: destination, source: source, routing_key: routing_key, nowait: nowait, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for BindOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<BindOk> {
...
impl Method for Unbind {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Unbind> {
        ...
        let nowait = bits.get(0);
        let arguments = decode_table(&mut reader).unwrap();
        Some(Unbind { ticket: ticket, destination: destination, source: source, routing_key: routing_key, nowait: nowait, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for UnbindOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<UnbindOk> {
...
impl Method for Declare {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Declare> {
        ...
        let ticket = reader.read_be_u16().unwrap();
        let size = reader.read_byte().unwrap() as uint;
        let queue = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        let passive = bits.get(0);
        let durable = bits.get(1);
        ...
        let nowait = bits.get(4);
        let arguments = decode_table(&mut reader).unwrap();
        Some(Declare { ticket: ticket, queue: queue, passive: passive, durable: durable, exclusive: exclusive, auto_delete: auto_delete, nowait: nowait, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for DeclareOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<DeclareOk> {
        ...
        let size = reader.read_byte().unwrap() as uint;
        let queue = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let message_count = reader.read_be_u32().unwrap();
        let consumer_count = reader.read_be_u32().unwrap();
        Some(DeclareOk { queue: queue, message_count: message_count, consumer_count: consumer_count })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_u8(self.queue.len() as u8).unwrap();
...
impl Method for Bind {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Bind> {
        ...
        let nowait = bits.get(0);
        let arguments = decode_table(&mut reader).unwrap();
        Some(Bind { ticket: ticket, queue: queue, exchange: exchange, routing_key: routing_key, nowait: nowait, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for Purge {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Purge> {
        ...
        let bits = bitv::from_bytes([byte]);
        let nowait = bits.get(0);
        Some(Purge { ticket: ticket, queue: queue, nowait: nowait })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for PurgeOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<PurgeOk> {
        if method_frame.class_id != 50 || method_frame.method_id != 31 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let message_count = reader.read_be_u32().unwrap();
        Some(PurgeOk { message_count: message_count })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u32(self.message_count).unwrap();
...
impl Method for Delete {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Delete> {
        ...
        Some(Delete { ticket: ticket, queue: queue, if_unused: if_unused, if_empty: if_empty, nowait: nowait })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for DeleteOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<DeleteOk> {
        ...
        let mut reader = MemReader::new(method_frame.arguments);
        let message_count = reader.read_be_u32().unwrap();
        Some(DeleteOk { message_count: message_count })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u32(self.message_count).unwrap();
...
impl Method for Unbind {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Unbind> {
        ...
        let routing_key = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let arguments = decode_table(&mut reader).unwrap();
        Some(Unbind { ticket: ticket, queue: queue, exchange: exchange, routing_key: routing_key, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for Qos {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Qos> {
        if method_frame.class_id != 60 || method_frame.method_id != 10 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let prefetch_size = reader.read_be_u32().unwrap();
        let prefetch_count = reader.read_be_u16().unwrap();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        let global = bits.get(0);
        Some(Qos { prefetch_size: prefetch_size, prefetch_count: prefetch_count, global: global })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for QosOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<QosOk> {
...
impl Method for Consume {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Consume> {
        ...
        let queue = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let size = reader.read_byte().unwrap() as uint;
        let consumer_tag = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        ...
        let arguments = decode_table(&mut reader).unwrap();
        Some(Consume { ticket: ticket, queue: queue, consumer_tag: consumer_tag, no_local: no_local, no_ack: no_ack, exclusive: exclusive, nowait: nowait, arguments: arguments })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for ConsumeOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<ConsumeOk> {
        ...
        let size = reader.read_byte().unwrap() as uint;
        let consumer_tag = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(ConsumeOk { consumer_tag: consumer_tag })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_u8(self.consumer_tag.len() as u8).unwrap();
...
impl Method for Cancel {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Cancel> {
        ...
        let bits = bitv::from_bytes([byte]);
        let nowait = bits.get(0);
        Some(Cancel { consumer_tag: consumer_tag, nowait: nowait })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_u8(self.consumer_tag.len() as u8).unwrap();
...
impl Method for CancelOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<CancelOk> {
        ...
        let size = reader.read_byte().unwrap() as uint;
        let consumer_tag = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(CancelOk { consumer_tag: consumer_tag })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_u8(self.consumer_tag.len() as u8).unwrap();
...
impl Method for Publish {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Publish> {
        ...
        Some(Publish { ticket: ticket, exchange: exchange, routing_key: routing_key, mandatory: mandatory, immediate: immediate })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for Return {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Return> {
        ...
        let size = reader.read_byte().unwrap() as uint;
        let routing_key = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(Return { reply_code: reply_code, reply_text: reply_text, exchange: exchange, routing_key: routing_key })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.reply_code).unwrap();
...
impl Method for Deliver {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Deliver> {
        ...
        let size = reader.read_byte().unwrap() as uint;
        let consumer_tag = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let delivery_tag = reader.read_be_u64().unwrap();
        let byte = reader.read_byte().unwrap();
        let bits = bitv::from_bytes([byte]);
        ...
        let size = reader.read_byte().unwrap() as uint;
        let routing_key = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(Deliver { consumer_tag: consumer_tag, delivery_tag: delivery_tag, redelivered: redelivered, exchange: exchange, routing_key: routing_key })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_u8(self.consumer_tag.len() as u8).unwrap();
...
impl Method for Get {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Get> {
        ...
        Some(Get { ticket: ticket, queue: queue, no_ack: no_ack })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u16(self.ticket).unwrap();
...
impl Method for GetOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<GetOk> {
        ...
        let routing_key = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        let message_count = reader.read_be_u32().unwrap();
        Some(GetOk { delivery_tag: delivery_tag, redelivered: redelivered, exchange: exchange, routing_key: routing_key, message_count: message_count })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u64(self.delivery_tag).unwrap();
...
impl Method for GetEmpty {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<GetEmpty> {
        if method_frame.class_id != 60 || method_frame.method_id != 72 {
           return None;
        }
        let mut reader = MemReader::new(method_frame.arguments);
        let size = reader.read_byte().unwrap() as uint;
        let cluster_id = String::from_utf8_lossy(reader.read_exact(size).unwrap().as_slice()).into_string();
        Some(GetEmpty { cluster_id: cluster_id })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
...
impl Method for Ack {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Ack> {
        ...
        Some(Ack { delivery_tag: delivery_tag, multiple: multiple })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u64(self.delivery_tag).unwrap();
...
impl Method for Reject {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Reject> {
        ...
        Some(Reject { delivery_tag: delivery_tag, requeue: requeue })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u64(self.delivery_tag).unwrap();
...
impl Method for RecoverAsync {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<RecoverAsync> {
        ...
        let bits = bitv::from_bytes([byte]);
        let requeue = bits.get(0);
        Some(RecoverAsync { requeue: requeue })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        let mut bits = Bitv::new();
...
impl Method for Recover {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Recover> {
...
impl Method for RecoverOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<RecoverOk> {
...
impl Method for Nack {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Nack> {
        ...
        Some(Nack { delivery_tag: delivery_tag, multiple: multiple, requeue: requeue })
    }
    fn encode(&self) -> Vec<u8> {
        let mut writer = MemWriter::new();
        writer.write_be_u64(self.delivery_tag).unwrap();
...
impl Method for Select {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Select> {
...
impl Method for SelectOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<SelectOk> {
...
impl Method for Commit {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Commit> {
...
impl Method for CommitOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<CommitOk> {
...
impl Method for Rollback {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<Rollback> {
...
impl Method for RollbackOk {
    ...
    fn decode(method_frame: protocol::MethodFrame) -> Option<RollbackOk> {
